...
| Action    | Description |
|-----------|-------------|
| `:create` | Writes the DNS config file and restarts `dnsmasq` if needed (default) |
...
| `disable_dhcp_interface` | Boolean | `true` | Adds `no-dhcp-interface=` when DHCP is not managed |
